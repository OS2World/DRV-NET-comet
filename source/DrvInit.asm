...
Credit		db	cr,lf,' ADMtek AL981/AN983 Another OS/2 NDIS MAC Driver '
		db	'ver.1.03 (2005-12-17)',cr,lf
Copyright	db	0	; Write copyright message here if you want.

Heap		db	( 84*(sizeof(vtxd) + 9*sizeof(txd)) \
			+ 84*(sizeof(rxd)+4) ) dup (?)
...
loc_1:
	...
	cmp	eax,9851317h		; AN983 centaur
	jz	short loc_2
	cmp	eax,9811317h		; AL981 comet
	jz	short loc_2
	cmp	eax,0a1201259h		; corega
	jnz	short loc_3
loc_2:
	mov	al,[FH_di]
	cmp	al,[FH_s]
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwTBC	; Warning: Invalid TBCNT
	jmp	short loc_ex
sci_TBCNT	endp
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwTTO	; Warning: Invalid TXTO
	jmp	short loc_ex
sci_TTO		endp
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwTAP	; Warning: Invalid TXAP
	jmp	short loc_ex
sci_TAP		endp
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwMWI	; Warning: Invalid PCIMWI
	jmp	short loc_ex
sci_MWI		endp
...
loc_ce:
loc_w:
	or	cfgKeyWarn,mask cwBAP	; Warning: Invalid PCIBAP
	jmp	short loc_ex
sci_BAP		endp
...
loc_0:
	...
	mov	ecx,[bp-10]
	mov	edx,1536
	mov	[bx].rxd.physadr,eax
	mov	[bx].rxd.buf,ecx
	add	dword ptr [bp-10],edx
	mov	word ptr [bx].rxd.sts[2],highword(OWN)
	mov	word ptr [bx].rxd.ctl,dx
	mov	word ptr [bx].rxd.ctl[2],highword(CHain)
	mov	di,[bp-4]
	mov	ax,[bp-6]
	mov	cx,[RxBufferSel][di]
	mov	word ptr [bx].rxd.vbuf,ax
	mov	word ptr [bx].rxd.vbuf[2],cx
	add	ax,dx
	cmp	ax,42*1536
	jb	short loc_1
	xor	ax,ax
...
loc_3:
	...
	mov	word ptr [bx].rxd.sts[2],0	; clear OWN, Terminate
...
loc_1:
	...
	pop	bx
	add	sp,3*2
	mov	[bx].txd.physadr,eax
	mov	word ptr [bx].txd.ctl[2],highword CHain
...
loc_3:
	...
	mov	di,[TxFreeHead]
	mov	[bx].txd.vlink,di	; ring
	mov	eax,[di].txd.physadr
	mov	[bx].txd.link,eax
